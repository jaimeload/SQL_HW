...
FROM shops
LEFT JOIN cats
ON shops.id = cats.shops_id AND name IN ('Murzik', 'Zuza')
...
# Не совсем понял просто список всех анализов или суммарно, потому ещё вот:
SELECT an_name, COUNT(an_name), SUM(an_price)
FROM analysis
JOIN orders
ON analysis.an_id = orders.ord_an
WHERE ord_datetime BETWEEN '2020-02-05 00:00:00' AND '2020-02-12 23:59:59'
